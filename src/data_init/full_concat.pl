...
$app_inserts    = "/home/bgovi/PsqlCred/output_data/sql_admin/app_inserts_all.psql";
$app_init        = '/home/bgovi/PsqlCred/output_data/app_init.psql';

# `cat  ./psql/psql_init.psql > ${app_init}`;

`(echo "DROP DATABASE IF EXISTS app;\nCREATE DATABASE app;\nBEGIN;\n"; cat  ./psql/psql_init.psql ${insert_path} ${roles_path} ${app_inserts} ${index_reset_path} ${update_trigger} ${app_user_trig}  )> ${app_init}`;
`echo "\nCOMMIT;\n" >> ${app_init}`